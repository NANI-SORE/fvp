...
extension FVPControllerExtensions on VideoPlayerController {
  ...
  bool isLive() {
    return _platform.isLive(playerId);
  }

  /// Get current media info.
  MediaInfo? getMediaInfo() {
    return _platform.getMediaInfo(playerId);
  }
  ...
  void setProperty(String name, String value) {
    _platform.setProperty(playerId, name, value);
  }
  ...
  void setVideoDecoders(List<String> value) {
    _platform.setVideoDecoders(playerId, value);
  }
  ...
  void record({String? to, String? format}) {
    _platform.record(playerId, to: to, format: format);
  }
  ...
  Future<Uint8List?> snapshot({int? width, int? height}) async {
    return _platform.snapshot(playerId, width: width, height: height);
  }
  ...
  void setRange({required int from, int to = -1}) {
    _platform.setRange(playerId, from: from, to: to);
  }
  ...
  void setBufferRange({int min = -1, int max = -1, bool drop = false}) {
    _platform.setBufferRange(playerId, min: min, max: max, drop: drop);
  }

  /// fast seek to a key frame
  Future<void> fastSeekTo(Duration position) async {
    return _platform.fastSeekTo(playerId, position);
  }

  /// Step forward or backward.
  /// Step forward if [frames] > 0, backward otherwise.
  Future<void> step({int frames = 1}) async {
    return _platform.step(playerId, frames);
  }

  /// set brightness. -1 <= [value] <= 1
  void setBrightness(double value) {
    _platform.setBrightness(playerId, value);
  }

  /// set contrast. -1 <= [value] <= 1
  void setContrast(double value) {
    _platform.setContrast(playerId, value);
  }

  /// set hue. -1 <= [value] <= 1
  void setHue(double value) {
    _platform.setHue(playerId, value);
  }

  /// set saturation. -1 <= [value] <= 1
  void setSaturation(double value) {
    _platform.setSaturation(playerId, value);
  }
  ...
  void setAudioTracks(List<int> value) {
    _platform.setAudioTracks(playerId, value);
  }

  /// Get active audio tracks.
  List<int>? getActiveAudioTracks() {
    return _platform.getActiveAudioTracks(playerId);
  }
  ...
  void setVideoTracks(List<int> value) {
    _platform.setVideoTracks(playerId, value);
  }

  /// Get active video tracks.
  List<int>? getActiveVideoTracks() {
    return _platform.getActiveVideoTracks(playerId);
  }
  ...
  void setSubtitleTracks(List<int> value) {
    _platform.setSubtitleTracks(playerId, value);
  }

  /// Get active subtitle tracks.
  List<int>? getActiveSubtitleTracks() {
    return _platform.getActiveSubtitleTracks(playerId);
  }
